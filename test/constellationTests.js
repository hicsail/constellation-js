...
module.exports = function() {
  ...
  describe('Basic operators', function() {
    it('atom', function() {
      let result = constellation(DESIGN_NAME, 'c', CATEGORIES, 10, 0);
      expect(result.designs.length).to.equal(CCATS);
      expect(result.designs).to.contain('c1');
    });

    it('or', function() {
      const result = constellation(DESIGN_NAME, 'a or b', CATEGORIES, 10, 0);
    ...
    it('then', function() {
      const result = constellation(DESIGN_NAME, 'a then c', CATEGORIES, 10, 0);
    ...
    it('then as dot', function() {
      const result = constellation(DESIGN_NAME, 'a . c', CATEGORIES, 10, 0);
      expect(result.designs.length).to.equal(ACATS * CCATS);
      expect(result.designs).to.contain('a1,c1');
      expect(result.designs).to.contain('a2,c1');
    });
    ...
    it('zero-or-more', function() {
      const result = constellation(DESIGN_NAME, 'zero-or-more a', CATEGORIES, 10, 0);
      expect(result.designs.length).to.equal(ACATS);
      expect(result.designs).to.contain('a1');
      expect(result.designs).to.contain('a2');
      expect(result.paths.length).to.equal(1);
  ...
  describe ('Chained expressions', function() {
    it('Multiple then', function() {
      const result = constellation(DESIGN_NAME, 'a then b then c', CATEGORIES, 10 , 0);
      expect(result.designs.length).to.equal(ACATS * BCATS * CCATS);
    });

    it('Multiple or', function() {
      const result = constellation(DESIGN_NAME, 'a or b or c', CATEGORIES, 10 , 0);
      expect(result.designs.length).to.equal(ACATS + BCATS + CCATS);
    });

    // it('Multiple and', function() {
    //   const result = constellation(DESIGN_NAME, 'a and b and c', categories, 10);
    //   expect(result.designs.length).to.equal(0);
    // });

    it('Multiple one-or-more', function() {
      const result = constellation(DESIGN_NAME, 'one-or-more (one-or-more a)', CATEGORIES, 10, 0);
      expect(result.designs.length).to.equal(ACATS);
      expect(result.designs).to.contain('a1');
      expect(result.designs).to.contain('a2');
      expect(result.paths.length).to.equal(1);
    ...
    it('Multiple zero-or-more', function() {
      const result = constellation(DESIGN_NAME, 'zero-or-more (zero-or-more a)', CATEGORIES, 10, 0);
      expect(result.designs.length).to.equal(ACATS);
      expect(result.designs).to.contain('a1');
      expect(result.designs).to.contain('a2');
    });
    ...
    it('Mixing functions', function() {
      const result = constellation(DESIGN_NAME, 'a then (one-or-more b or zero-or-more c)', CATEGORIES, 50, 0); // TODO add and
      expect(result.designs.length).to.equal(ACATS * (BCATS + CCATS + 1));
    });

    it('Then downstream from cycle', function() {
      const result = constellation(DESIGN_NAME, 'zero-or-more a then b', CATEGORIES, 50, 0);
  ...
  describe('Cycles', function () {
    it('Atom', function() {
      let result = constellation(DESIGN_NAME, 'c', CATEGORIES, 10, 2);
      expect(result.designs.length).to.equal(CCATS);
    });

    it('Linear operators', function() {
      let result = constellation(DESIGN_NAME, 'a or b', CATEGORIES, 10, 2);
      expect(result.designs.length).to.equal(ACATS + BCATS);
      expect(result.designs).to.contain('a1');
      expect(result.designs).to.contain('a2');
      expect(result.designs).to.contain('b1');
      expect(result.designs).to.contain('b2');
      expect(result.designs).to.contain('b3');
      result = constellation(DESIGN_NAME, 'a then c', CATEGORIES, 10, 2);
      expect(result.designs.length).to.equal(ACATS * CCATS);
      expect(result.designs).to.contain('a1,c1');
      expect(result.designs).to.contain('a2,c1');
    });
    ...
    it('one-or-more', function() {
      let result = constellation(DESIGN_NAME, 'one-or-more a', CATEGORIES, 10, 1);
    ...
    it('zero-or-more', function() {
      const result = constellation(DESIGN_NAME, 'zero-or-more a', CATEGORIES, 10, 1);
      expect(result.designs.length).to.equal(ACATS + ACATS * ACATS);
      expect(result.designs).to.contain('a1');
      expect(result.designs).to.contain('a2');
      expect(result.designs).to.contain('a1,a1');
    ...
    it('Multiple one-or-more', function() {
      const result = constellation(DESIGN_NAME, 'one-or-more (one-or-more a)', CATEGORIES, 10, 1);
      expect(result.designs.length).to.equal(ACATS + ACATS * ACATS);
      expect(result.designs).to.contain('a1');
      expect(result.designs).to.contain('a2');
      expect(result.designs).to.contain('a1,a1');
    ...
    it('Multiple zero-or-more', function() {
      const result = constellation(DESIGN_NAME, 'zero-or-more (zero-or-more a)', CATEGORIES, 10, 1);
      expect(result.designs.length).to.equal(ACATS + ACATS * ACATS);
      expect(result.designs).to.contain('a1');
      expect(result.designs).to.contain('a2');
      expect(result.designs).to.contain('a1,a1');
    ...
    it('Then downstream from cycle', function() {
      const result = constellation(DESIGN_NAME, 'zero-or-more a then b', CATEGORIES, 50, 1);
  ...
  describe('Sanitise specification input', function () {
    it('Atom not in categories', function () {
      expect(() => constellation(DESIGN_NAME, 'd', CATEGORIES, 10, 0)).to.throw('d is not defined in categories');
    });

    it('Mismatched brackets', function () {
      expect(() => constellation(DESIGN_NAME, '(a}', CATEGORIES, 10, 0)).to.throw('Parsing error!');
    });


    it('Empty specification', function () {
      expect(() => constellation(DESIGN_NAME, '', CATEGORIES, 10, 0)).to.throw('No input received')
    });

    describe('Invalid characters', function () {
      it('Tabs used should not throw errors', function () {
        const result = constellation(DESIGN_NAME, '\ta', CATEGORIES, 10, 0);
        expect(result.designs).to.contain('a1');
        expect(result.designs).to.contain('a2');
      });

      // it('$', function () {
      //   expect(() => constellation(DESIGN_NAME, 'a then $a', CATEGORIES, 10)).to.throw('Parsing error!');
      // });
      // TODO turn back on when imparse starts throwing errors

      it('_', function () {
        expect(() => constellation(DESIGN_NAME, '_a', CATEGORIES, 10, 0)).to.throw('_a is not defined in categories');
      });
    });
  ...
  describe('Sanitise category input', function () {
    it('Empty categories', function () {
      const categories = '{}';
      expect(() => constellation(DESIGN_NAME, 'a', categories, 10, 0)).to.throw('a is not defined in categories');
    });
