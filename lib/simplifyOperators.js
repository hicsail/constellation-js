...
/**
 * Checks if both sides of the operator tree are symmetric
 * @param parsed string using imparse library
 */
function traverseTree(parsed) {

  // If operation is just Atom
  if (parsed.Atom) {
    console.log(parsed['Atom']);
    return;
  }

  // Or cases are arrays. This should be changed as the array length should be no more than 2
  if (Array.isArray(parsed)) {
    for (let i = 0; i < parsed.length; i++) {
      console.log('array', parsed[i]);
      traverseTree(parsed[i]);
    }
    return;
  }

  // Nested operations case
  if (Object.keys(parsed).length === 1) {
    for (let operation in parsed) {
      console.log(operation, parsed[operation]);
      traverseTree(parsed[operation])
    }
  }
}
...
function simplifyThen(exp1, exp2) {
  // console.log("exp1 then exp2", JSON.stringify(exp1), JSON.stringify(exp2));
...
function simplifyZeroOrMore(exp) {
  if (exp.OneOrMore) {
    return ZeroOrMore(exp.OneOrMore);
