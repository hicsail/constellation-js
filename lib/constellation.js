...
function goldbar(designName, langText, categories, numDesigns, maxCycles = 1, representation) {
  ...
  try {
    ...
    // langText = langText.replace(/zero-or-one/gi, 'zero-or-one-sbol');
    // sparsedSBOL = parseGoldbar(langText.replace(/zero-or-more/gi, 'zero-or-more-sbol'));
  ...
  console.log("simple parsed", JSON.stringify(sparsed));
  sparsedSBOL = simplify.getSimplifiedSBOL(sparsed);
  console.log("simple parsed SBOL", JSON.stringify(sparsedSBOL));

  if (representation === NODE) {
    gra = graphGOLDBAR(sparsed, maxCycles, NODE);
    graSBOL = graphGOLDBAR(sparsedSBOL, maxCycles, NODE).stateGraph;
    // console.log(JSON.stringify(gra.stateGraph, null, 2));
    designs = design.enumerateDesigns(gra.paths, categories, numDesigns);

  } else if (representation === EDGE) {
    gra = graphGOLDBAR(sparsed, maxCycles, EDGE);
    graSBOL = graphGOLDBAR(sparsedSBOL, maxCycles, NODE).stateGraph;
    designs = design.enumerateDesigns(gra.paths, categories, numDesigns);
  } else {
