...
function getRootId(stateGraph){
  for (let id in stateGraph) {
    if (stateGraph[id].text === constants.ROOT){
      return id;
    }
  }
}
...
function makeStackFromStateGraph(stateGraph, stateStack, id, edgeUsed){
  ...
  while (queue.length !== 0){
    ...
    if (stateGraph[id].visited){
      ...
      if (stateGraph[id].operator.includes(constants.ZERO_MORE) || stateGraph[id].operator.includes(constants.ONE_MORE)) {
        if (!endIds.includes(id)) {
          stateStack.push({end: id});
          endIds.push(id);
        } else if (stateGraph[id].operator.filter(op => op === constants.ONE_MORE || op === constants.ZERO_MORE).length > 1) {
          stateStack.push({end: id});
          endIds.push(id);
        }
      }
...
function handleOperations(stateGraph, stack, id, endIds, edgeUsed) {
  let lastEdge;
  // handle THENs first
  if (stateGraph[id].operator.includes(constants.THEN)) {
...
function traverseOrEdges(stateGraph, stack, subStack, id, edgeUsed, endIds, prevEdgeUsed, moreStartId) {
  if (stateGraph[id].visited) {
    ...
    if (id === moreStartId) {
      ...
      || stateGraph[id].operator.includes(constants.ZERO_MORE)
      || stateGraph[id].operator.includes(constants.ZERO_ONE)) {
  ...
  if (stateGraph[id].operator.length === 1 &&
    (stateGraph[id].operator[0] === constants.ONE_MORE ||
      stateGraph[id].operator[0] === constants.ZERO_MORE ||
      stateGraph[id].operator[0] === constants.ZERO_ONE)) {
    moreStartId = id;
  }

  if (edgeUsed.type === constants.EPSILON && stateGraph[id].operator.includes(constants.THEN)) {
