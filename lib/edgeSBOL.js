...
function makeStackFromStateGraph(stateGraph, stateStack, id, edgeUsed){
  ...
  while (queue.length !== 0){
    ...
    if (stateGraph[id].visited){
      ...
      if ((stateGraph[id].operator.includes(handleOp.ZERO_MORE) || stateGraph[id].operator.includes(handleOp.ONE_MORE)) && (!endIds.includes(id))) {
      // if (edgeUsed.type !== handleOp.ATOM && !endIds.includes(id)){
    ...
    queue.push(...dests);
    edgeQueue.push(...stateGraph[id].edges); // push edges
  }
}

function handleOperations(stateGraph, stack, id, endIds) {
  // console.log('in HANDLE ', pMap);
  let lastEdge;
  // handle THENs first
  ...
  for (let operation of stateGraph[id].operator) {
    if (operation === handleOp.OR) {
      let temp = traverseOr(stateGraph, stack, id, endIds);
...
function traverseOr(stateGraph, stateStack, id, endIds) {
  ...
  stateGraph[id].edges.forEach(function (edge) {
    let orSubStack = [];
    let temp = traverseOrEdges(stateGraph, stateStack, orSubStack, edge.dest, edge, endIds);
    if (temp !== undefined && temp !== null) {
      lastEdge = temp;
    }
...
function traverseOrEdges(stateGraph, stack, subStack, id, edgeUsed, endIds, moreStartId) {
  // console.log('in TRAV_E ', pMap);
  ...
  for (let edge of stateGraph[id].edges) {
    lastEdge = traverseOrEdges(stateGraph, stack, subStack, edge.dest, edge, endIds, moreStartId);
  }
  // if (stateGraph[id].type === handleOp.ACCEPT) {
  //   return id;
  // }
