...
function populateGraph(parsed, stateGraph, boundaryStack, representation, categories, maxCycles) {
  ...
  if (parsed.And) {
    ...
    for (let i = 0; i < parsed.And.length; i++) {
      let partialGraph = {};
      let partialBoundary = [];
      populateGraph(parsed.And[i], partialGraph, partialBoundary, representation, categories);
    ...
    let combined = combineGraphs.combineGraphs('and', partialStateGraphs, artificialCategories);
