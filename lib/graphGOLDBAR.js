...
function populateGraph(parsed, stateGraph, boundaryStack, representation, categories, maxCycles) {
  ...
  if (Object.keys(parsed).length > 0) {
    for (let operation in parsed) {
      ...
      if (operation === handleOp.AND0) {
        handleOp.handleOp(handleOp.AND, stateGraph, boundaryStack, representation, populateArgs, 0);
        return;
      } else if (operation === handleOp.AND1) {
        handleOp.handleOp(handleOp.AND, stateGraph, boundaryStack, representation, populateArgs, 1);
        return;
      } else if (operation === handleOp.AND2) {
        handleOp.handleOp(handleOp.AND, stateGraph, boundaryStack, representation, populateArgs, 2);
        return;
