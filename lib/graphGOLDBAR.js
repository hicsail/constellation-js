...
function createGraphFromGOLDBAR(parsedObject, maxCycles, representation, categories) {
  ...
  populateGraph(parsedObject, stateGraph, boundaryStack, representation, categories, maxCycles);
 ...
 */
function populateGraph(parsed, stateGraph, boundaryStack, representation, categories, maxCycles) {
  ...
  if (Array.isArray(parsed)) {
    for (let i = 0; i < parsed.length; i++) {
      populateGraph(parsed[i], stateGraph, boundaryStack, representation, categories, maxCycles);
    }
    return;
  }
  ...
  if (Object.keys(parsed).length > 0) {
    for (let operation in parsed) {
      ...
      if (operation === constants.AND0) {
        handleOp.handleOp(constants.AND, stateGraph, boundaryStack, representation, populateArgs, 0);
        return;
      } else if (operation === constants.AND1) {
        handleOp.handleOp(constants.AND, stateGraph, boundaryStack, representation, populateArgs, 1);
        return;
      } else if (operation === constants.AND2) {
        handleOp.handleOp(constants.AND, stateGraph, boundaryStack, representation, populateArgs, 2);
        return;
        ...
        handleOp.handleOp(constants.MERGE, stateGraph, boundaryStack, representation, populateArgs);
        return;
      } else {
        populateGraph(parsed[operation], stateGraph, boundaryStack, representation, categories, maxCycles);
        handleOp.handleOp(operation, stateGraph, boundaryStack, representation, populateArgs);
      }
    }
  }
