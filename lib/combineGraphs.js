...
const CYCLES = [ONE_MORE, ZERO_MORE, ZERO_ONE];
...
function handleAnd(graph1, graph2, categories1, categories2, tolerance) {
  ...
  propagateBlanks(graph1, newGraph, 0, 1, tensorNodes);
  propagateBlanks(graph2, newGraph, 1, 0, tensorNodes);
...
function cartesianNodes(graph1, graph2) {
  ...
  for (let id1 in graph1) {
    ...
    for (let id2 in graph2) {
      ...
      if (node1.operator.length !== 0 && node2.operator.length !== 0) {
...
function assignOperators(opArr1, opArr2) {
  let finalOps = [];
  let nonCycles1 = opArr1.filter(value => !CYCLES.includes(value));
  let nonCycles2 = opArr2.filter(value => !CYCLES.includes(value));
  let cycles1 = opArr1.filter(value => CYCLES.includes(value));
  let cycles2 = opArr2.filter(value => CYCLES.includes(value));
  finalOps = finalOps.concat(nonCycles1.concat(nonCycles2));

  let compare = [highestCycle(cycles1), highestCycle(cycles2)];
  let keepCycle = highestCycle(compare);
  if (keepCycle !== '') {
    finalOps.push(keepCycle);
  }
  return [...new Set(finalOps)]

}

function highestCycle(cycleArr) {
  if (cycleArr.includes(ZERO_ONE)) {
    return ZERO_ONE;
  } else if (cycleArr.includes(ONE_MORE)) {
    return ONE_MORE;
  } else if (cycleArr.includes(ZERO_MORE)) {
    return ZERO_MORE;
  } else {
    return '';
  }
 ...
 */
function propagateBlanks(oldGraph, newGraph, idPos, notIDPos, tensorNodes) {
      ...
      if (edge.component === EPSILON) {
        for (let newID in newGraph) {
          ...
          if (newNode.id[idPos] === edge.src) {
            ...
            if ((newNode.operator.filter(value => CYCLES.includes(value)).length !== 0) && CYCLES.includes(edge.type)) {
              newEdge = {
                src: newNode.id,
                dest: newDest,
                component: EPSILON,
                type: newNode.operator.filter(value => CYCLES.includes(value))[0],
                text: edge.text
              };
            } else if ((newGraph[newDest].operator.filter(value => CYCLES.includes(value)).length !== 0) && CYCLES.includes(edge.type)) {
              newEdge = {
                src: newNode.id,
                dest: newDest,
                component: EPSILON,
                type: newGraph[newDest].operator.filter(value => CYCLES.includes(value))[0],
                text: edge.text
              };
            } else {
              newEdge = {
