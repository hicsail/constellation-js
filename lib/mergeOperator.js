    ...
    for (let edge1 of graph1[id1].edges) {
      if (edge1.component !== constants.EPSILON) {
        for (let edge2 of graph2[id2].edges) {
          ...
          if (edge2.component !== constants.EPSILON) {
...
function propagateHelper(newGraph, edge, idPos, notIDPos, cartesianOnly, tensorNodes, edgeLabel) {
  for (let newID in newGraph) {
    ...
    if (newNode.id[idPos] === edge.src) {
      ...
      if ((edge.component === constants.EPSILON) && (newGraph[newNode.id].operator.length === 0) && (newGraph[newDest].operator.length === 0)) {
        newEdge = {
          src: newNode.id,
          dest: newDest,
...
function createsFalseCycle(graph, nextEdge, path) {
  ...
  if (last.src === nextEdge.dest) {
    ...
    if (!cycleHead.operator.includes(constants.ONE_MORE) && !cycleHead.operator.includes(constants.ZERO_MORE)) {
      return true;
