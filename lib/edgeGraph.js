...
class EdgeGraph extends StateGraph {
  ...
  handleOr(boundaryStack, representation, populateArgs) {
    ...
    for (let i = 0; i < populateArgs.parsed.length; i++) {
      let partialBoundary = [];
      handleOp.populateGraph(populateArgs.parsed[i], this, partialBoundary, representation, populateArgs.categories, populateArgs.maxCycles);
  ...
  handleAnd(boundaryStack, representation, populateArgs, tolerance) {
    ...
    for (let i = 0; i < populateArgs.parsed.length; i++) {
      ...
      let partialBoundary = [];
      handleOp.populateGraph(populateArgs.parsed[i], partialGraph, partialBoundary, representation, populateArgs.categories, populateArgs.maxCycles);
  ...
  handleAtom(atom, boundaryStack, categories) {
    ...
    this.addAtomEdge(epsilon0, epsilon1, categories[atom[0]], atom[0]);
