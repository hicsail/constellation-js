...
function generateDesigns(paths, categories, buffer) {
  ...
  for (const path of paths) {
    ...
    for (let j = 0; j < path.length; j++) {
      ...
      // add 'reverse-comp()' to reverse complemented part IDs for display
      let ids = getAllIDs(pathCats[tempNode]);
      if (path[j].orientation === constants.REV_COMP) {
        for (let i = 0; i < ids.length; i++) {
          ids[i] = `reverse-comp(${ids[i]})`;
        }
      }

      currentProduct = getCartesianProduct(currentProduct, ids);
    }
