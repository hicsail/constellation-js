...
function generateDesigns(paths, categories) {
  ...
  for (const path of paths) {
    ...
    for (let j = 0; j < path.length; j++) {
      ...
      // if (!(tempNode in SBOLDocument.terms)) {
      //   throw new Error(`${tempNode} is not a valid SBOL part`);
      // }
      if (!(tempNode in categories)) {
        throw new Error(`${tempNode} is not defined in categories`);
      }

      currentProduct = getCartesianProduct(currentProduct, getAllIDs(categories[tempNode]));
    }
