...
function generateDesigns(paths, categories) {
  ...
  for (const path of paths) {
    ...
    // Check for empty paths, which contains only the root node
    if (path.length === 1) {
    ...
    for (let j = 1; j < path.length; j++) {
      const tempNode = path[j].data.text;

      if (!tempNode in categories) {
        return generateError(`${tempNode} is not defined in categories`);
      }
...
/**
 * Adds an array of generated designs to an existing array
 * @param designs Array of designs
 * @param collection Existing array
 * @returns {*}
 */
...
/**
 * Generates error based on error message string
...
/**
 * Checks if a node is defined in categories
 * @param node The nodeto check
 * @param categories The category to check to
 * @returns boolean Whether the node is defined in categories
 */
function isInCategories(node, categories) {
