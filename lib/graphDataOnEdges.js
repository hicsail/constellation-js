...
function enumeratePaths(root, stateGraph, maxCycles) {
  ...
  for (let path of allPaths) {
    ...
    //   console.log(path[i].src + "_" + path[i].dest, path[i].text);
    // }
    printPath(path);
  }
  return allPaths;
...
function visitEdges(nodeId, edgeUsed, visited, stateGraph, currentPath, allPaths, maxCycles) {
  ...
  const edgeId = edgeUsed.src + "_" + edgeUsed.dest;

  // Don't let atoms exceed max cycle depth
  if (visited[edgeId] > maxCycles && edgeUsed.type === ATOM) {
    return;
  }
...
function isDuplicatePath(processedPath, allPaths) {
  for (let path of allPaths) {
    ...
    for (let i = 0; i < processedPath.length && equal; i++) {
      if (processedPath[i].src !== path[i].src || processedPath[i].dest !== path[i].dest) {
        equal = false;
      }
    }
  ...
  return true; // this means it is NOT a duplicate path
}
