...
function handleAnd(stateGraph, boundaryStack, representation, populateArgs, tolerance) {
  ...
  let combined = combineGraphs.combineGraphs(combineGraphs.AND, partialStateGraphs, artificialCategories, tolerance);
...
function handleMerge(stateGraph, boundaryStack, representation, populateArgs) {
  ...
  // combine the graphs and add them to the larger stateGraph of the whole expression
  let combined = combineGraphs.combineGraphs(combineGraphs.MERGE, partialStateGraphs, artificialCategories);
  let combinedRoot = combineGraphs.findRoot(combined.graph);
  let combinedAccepts = combineGraphs.findAccepts(combined.graph);
