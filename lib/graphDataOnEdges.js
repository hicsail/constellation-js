...
function enumeratePaths(root, stateGraph, maxCycles) {
  ...
  pruneEpsilonFamily(stateGraph, pMap, cMap);
  collapseEpsilons(stateGraph, pMap, cMap);

  removeDuplicateEdges(stateGraph);

  let dummyEdge = {'src': 'dummy', 'dest': root, 'component': EPSILON, 'type': EPSILON, 'text': EPSILON};
  visitEdges(root, dummyEdge, visited, stateGraph, [], allPaths, maxCycles);
...
function visitEdges(nodeId, edgeUsed, visited, stateGraph, currentPath, allPaths, maxCycles) {
  ...
  for (let e of node.edges) {
    visitEdges(e.dest, e, visited, stateGraph, currentPath, allPaths, maxCycles);
  }
  currentPath.pop();
  visited[edgeId]--;
}
...
function removeDuplicateEdges(stateGraph) {

  for (let node in stateGraph) {
    edgeSet = new Set();
    for (let edge of stateGraph[node].edges) {
      let info = JSON.stringify({dest: edge.dest, type: edge.text});
      if (edgeSet.has(info)) {
        let index = stateGraph[node].edges.indexOf(edge);
        if (index > -1) {
          stateGraph[node].edges.splice(index, 1);
        }
      } else {
        edgeSet.add(info);
      }
...
function pruneEpsilonFamily(stateGraph, pMap, cMap) {
  for (let src in stateGraph) {
    for (let edge of stateGraph[src].edges) {
      for (let de of stateGraph[edge.dest].edges) {
        if (de.dest === src && de.dest in pMap) {
          delete pMap[de.dest];
          delete cMap[de.dest];
        }
      }
