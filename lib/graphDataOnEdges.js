...
function enumeratePaths(root, stateGraph, maxCycles) {
  ...
  getAllParents(stateGraph, eMap);
  collapseEpsilons(stateGraph, eMap);
 ...
 */
function collapseEpsilons(stateGraph, eMap) {
...
/**
 * Maps epsilon edge's src and destination.
 * @param stateGraph Current graph
...
function getAllParents(stateGraph, eMap) {
  for (let srcNode in stateGraph) {
    for (let edge of stateGraph[srcNode].edges) {
      if (edge.dest in eMap) {
        eMap[edge.dest].add(edge.src);
  ...
  console.log("getAllParents", eMap);
}
