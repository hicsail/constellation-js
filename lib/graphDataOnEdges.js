...
function enumeratePaths(root, stateGraph, maxCycles) {
  ...
  console.time('visitEdges');
  allPaths = visitEdges(root, dummyEdge, stateGraph, maxCycles);
  //visitEdges2(root, dummyEdge, visitedE, visitedN, stateGraph, [], allPaths, maxCycles);
  console.timeEnd('visitEdges');
...
function visitEdges(root, dummyEdge, stateGraph, maxCycles) {
  // All found paths
  let allPaths = [];
  // Current path in BFS
  let currentPath = [];
  // Same as current path, but using concatenated edge src and destination
  // Used to ensure cycles aren't added
  let edgeCurrentPath = [];
  // List of currentPaths, yet to process
  let queue = [];
  // List of edgeCurrentPaths, yet to process
  let edgeQueue = [];

  queue.push([root]);
  edgeQueue.push([dummyEdge.src + "_" + dummyEdge.dest + "_" + dummyEdge.text])

  while (queue.length) {
    // Get first path from queue to process
    currentPath = queue.shift();
    edgeCurrentPath = edgeQueue.shift();

    // We get edges from the last node on this path
    const lastNode = currentPath[currentPath.length - 1];

    // If the last node is of type ACCEPT, we've reached a valid path
    if (stateGraph[lastNode].type === handleOp.ACCEPT) {
      // We only store node IDs in currentPath, not the edge objects which have extra information
      // processPath requires this
      let newPath = [];
      newPath.push(dummyEdge);
      // Loop through all nodes and get their edges
      currentPath.forEach((val, i, arr) => {
        for (let edge of stateGraph[val].edges) {
          // Then make sure that the edge destination matches the next node in our path
          if (edge.dest === currentPath[i+1]) {
            newPath.push(edge);
          }
        }
      });

      processPath(newPath, allPaths);
      // console.log('valid path', allPaths[allPaths.length - 1]);
    }

    // Add next set of edges to queue to be processed
    for (const edge of stateGraph[lastNode].edges) {
      // Ensure there are no cycles for certain node type, node should only appear once in currentPath
      if (stateGraph[lastNode].operator.includes(handleOp.ONE_MORE) && (currentPath.indexOf(lastNode) !== currentPath.lastIndexOf(lastNode))) {
        continue;
      }

      // Also ensure edge hasn't been added before, then push to queue
      const edgeId = edge.src + "_" + edge.dest + "_" + edge.text;
      if (!(edgeCurrentPath.includes(edgeId))) {
        const newPath = currentPath.concat([edge.dest]);
        const newPathEdge = edgeCurrentPath.concat([edgeId]);
        queue.push(newPath);
        edgeQueue.push(newPathEdge);
      }
    }
  }
  return allPaths;
...
function visitEdges2(nodeId, edgeUsed, visitedE, visitedN, stateGraph, currentPath, allPaths, maxCycles) {
  ...
  for (let e of node.edges) {
    visitEdges2(e.dest, e, visitedE, visitedN, stateGraph, currentPath, allPaths, maxCycles);
  }
  currentPath.pop();
  visitedE[edgeId]--;
  visitedN[nodeId]--;
