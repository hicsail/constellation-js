...
function addAcceptNodes(stateGraph, boundaryStack) {
  ...
  for (var leafId of leaves) {
    stateGraph[leafId].type = constants.ACCEPT;
    stateGraph[leafId].text = constants.ACCEPT;
  }
}

function createEpsilonNode(stateGraph) {
  const epsilonId = uuidv4();
  stateGraph[epsilonId] = {id: epsilonId, text: constants.EPSILON, type: constants.EPSILON, edges: [], operator: []};
  return epsilonId;
}
...
function generateRootNode(stateGraph, boundaryStack) {
  ...
  if (stateGraph[root].type === constants.ACCEPT) {
    stateGraph[root].text = constants.ROOT;
  } else {
    stateGraph[root].text = constants.ROOT;
    stateGraph[root].type = constants.ROOT;
  }
  return root;
}
...
function handleOr(stateGraph, boundaryStack, representation, populateArgs) {
  ...
  while (partialBoundaryStack.length > 0) {
    ...
    stateGraph[parentId].edges.push({'src': parentId, 'dest': orObj.head, 'component': constants.EPSILON, 'type': constants.EPSILON, 'text': constants.EPSILON});
    for (let leaf of orObj.leaves) {
      leaves.push(leaf);
    }
  }

  stateGraph[parentId].operator.push(constants.OR);
...
function handleOrSBOL(stateGraph, partialBoundary) {
  ...
  while (partialBoundary.length > 0) {
    ...
    stateGraph[parentId].edges.push({'src': parentId, 'dest': orPart.head, 'component': constants.EPSILON, 'type': constants.EPSILON, 'text': constants.EPSILON});
 ...
 */
function handleAnd(stateGraph, boundaryStack, representation, populateArgs, tolerance) {
  ...
  for (let i = 0; i < populateArgs.parsed.length; i++) {
    let partialGraph = {};
    let partialBoundary = [];
    graphGOLDBAR.populateGraph(populateArgs.parsed[i], partialGraph, partialBoundary, representation, populateArgs.categories, populateArgs.maxCycles);
...
function handleThen(stateGraph, boundaryStack) {
  ...
  if (stateGraph[a.head].operator.includes(constants.OR)) {
    let lenA = a.leaves.length;
    for (let i = 0; i < lenA; i++) {
      const leaf = a.leaves.pop();
      stateGraph[leaf].edges.push({'src': leaf, 'dest': b.head, 'component': constants.EPSILON, 'type': constants.EPSILON, 'text': constants.EPSILON});
    }
    stateGraph[b.head].operator.push(constants.THEN);
  } else {
    ...
    stateGraph[leaf].operator.push(constants.THEN);
...
function handleZeroOrMore(stateGraph, boundaryStack) {
  ...
  for (let leaf of a.leaves) {
    stateGraph[leaf].edges.push({'src': leaf,
      'dest': a.head,
      'component': constants.EPSILON,
      'type': constants.ZERO_MORE,
      'text': constants.OR_MORE });
  }

  stateGraph[a.head].operator.push(constants.ZERO_MORE);
...
function handleZeroOrOne(stateGraph, boundaryStack) {
  ...
  for (let leaf of a.leaves) {
    stateGraph[a.head].edges.push({'src': a.head,
      'dest': leaf,
      'component': constants.EPSILON,
      'type': constants.ZERO_ONE,
      'text': 'ZERO'
    });
  }

  stateGraph[a.head].operator.push(constants.ZERO_ONE);
...
function handleOneOrMore(stateGraph, boundaryStack) {
  ...
  for (var leaf of a.leaves) {
    stateGraph[leaf].edges.push({'src': leaf,
      'dest': a.head,
      'component': constants.EPSILON,
      'type': constants.ONE_MORE,
      'text': constants.OR_MORE
    });
  }

  stateGraph[a.head].operator.unshift(constants.ONE_MORE);
  addToBoundaryStack(a.head, a.leaves, boundaryStack);
}
...
function handleZeroOrMoreSbol(stateGraph, boundaryStack) {
  ...
  stateGraph[a.head].operator.push(constants.ZERO_SBOL);
  addToBoundaryStack(a.head, a.leaves, boundaryStack);
}
...
function nextEdges(graph, edge, path, maxCycles) {
  ...
  for (let nextEdge of graph[edge.dest].edges) {
    ...
    if (nextNode.operator.includes(constants.ONE_MORE)) {
...
function processPath(graph, node, path, allPaths) {
  ...
  for (let edge of path) {
    if (edge.component === constants.EPSILON || node.type === constants.ROOT) {
      continue;
    }
...
function collapseEpsilons(stateGraph){
  ...
  for (let node in stateGraph) {
    ...
    if (stateGraph[node].type === constants.EPSILON) {
...
function collapseNode(stateGraph, pMap, node) {
  ...
  for (let i = 0; i < parentIds.length; i++) {
    ...
    if (stateGraph[node].edges[0].type === constants.ATOM){
