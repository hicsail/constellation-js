 ...
 * @param representation representation style of graph (NODE or EDGE)
 * @param populateArgs arguments to populateGraph (from graphGOLDBAR)
 */
function handleOr(stateGraph, boundaryStack, parentId, representation, populateArgs) {
  let leaves = [];
  ...
  while (partialBoundaryStack.length > 0) {
    ...
    stateGraph[parentId].edges.push(orObj.head);
