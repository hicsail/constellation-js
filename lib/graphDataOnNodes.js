...
function addAcceptNodes(stateGraph, boundaryStack) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    stateGraph[acceptId] = {id: acceptId, text: ACCEPT, type: ACCEPT, component: EPSILON, edges:[], operator: []};
...
function handleOp(op, stateGraph, boundaryStack) {
  ...
  // console.log('BEFORE HANDLE ---->>> ', util.inspect(stateGraph, {showHidden: false, depth: null}));
  if(op === 'one'){
    return;
  }
  ...
  if (op === ZERO_ONE) {
    handleZeroOrOne(stateGraph, boundaryStack, parentId);
  }
  if (op === ONE_MORE) {
  ...
  if (op === ZERO_ONE_SBOL) {
    handleZeroOrOneSbol(stateGraph, boundaryStack, parentId);
  }
...
function handleZeroOrOne(stateGraph, boundaryStack, parentId) {
  ...
  stateGraph[epsilonId] = {id: epsilonId, text: EPSILON, type: EPSILON, component: ZERO_ONE, edges: [], operator: []};
  ...
  stateGraph[parentId].operator.push(ZERO_ONE);
...
function handleOneOrMore(stateGraph, boundaryStack, parentId) {
  ...
  stateGraph[a.head].operator.unshift(ONE_MORE);
...
function handleZeroOrMoreSbol(stateGraph, boundaryStack, parentId) {
  ...
  const epsilonId = uuidv4();
  stateGraph[epsilonId] = {id: epsilonId, text: EPSILON, type: EPSILON, component: ZERO_SBOL, edges:[], operator: []};
  stateGraph[parentId].edges.push(a.head);
...
function handleZeroOrOneSbol(stateGraph, boundaryStack, parentId) {
  ...
  const epsilonId = uuidv4();
  stateGraph[epsilonId] = {id: epsilonId, text: EPSILON, type: EPSILON, component: EPSILON, edges:[], operator: []};
  stateGraph[parentId].edges.push(a.head);
  ...
  stateGraph[parentId].operator.push(ZERO_ONE_SBOL);
  stateGraph[epsilonId].edges.push(parentId);
  addToBoundaryStack(parentId, tempLeaves, boundaryStack);
}
...
function handleAtom(atom, stateGraph, boundaryStack, categories) {
  ...
  stateGraph[epsilonId] = {id: epsilonId, text: EPSILON, type: EPSILON, component: EPSILON, edges: [atomId], operator: []};
  stateGraph[atomId] = {id: atomId, text: atom[0], type: ATOM, component: categories[atom[0]], edges: [], operator: []};
    ...
    for (let child of epsilon.edges) {
      ...
      if (epsilon.operator !== undefined ) {
        ...
        for (var i = 0; i < epsilon.operator.length; i++) {
          if (epsilon.operator[0] === ZERO_ONE && parent.operator[last] === ZERO_ONE) {
            continue;
          }
