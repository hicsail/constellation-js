...
class sbolGeneration {
  ...
  constructor(designName) {
    ...
    this.sequenceAnnotationCount = 1;
  ...
  static createVariantComponentObj(operator, name, component, variantDVs, collections, orientation=constants.INLINE_URI){
    return {
      operator: operator,
      name: name,
      component: component,
  ...
  static createComponentDefObj(operator, componentDef, variantDVs, collections, name, orientation=constants.INLINE_URI){
    return {
      operator: operator,
  ...
  addAtomToStacks(stacks, stackObj, operator, variantDerivations, collections){
    let name = stackObj[constants.ATOM];
    if (!collections){
      collections = [];
      if (this.atomMap[name]){
        collections = this.atomMap[name].collections;
    ...
    if (stacks.templates.length > 0){
      ...
      // else, this is a new componentDef for the root
      stacks.componentDefs.push(sbolGeneration.createComponentDefObj(operator, componentDefinition, variantDerivations, collections, name, stackObj.orientation));
    }
  }
  ...
  addComponentsAndRestraintsToTemplate(templateCD, stack, cv, name=null, makeRoot=false){
    ...
    for(let stackObj of stack){
      ...
      // add sequence annotation for reverse comp parts
      if (stackObj.name in this.atomMap) {
        let seqAn = this.makeSequenceAnnotation(stackObj.orientation, templateCD.displayId, templateCD.persistentIdentity);
        seqAn.component = component;
        templateCD.addSequenceAnnotation(seqAn);
      }
  ...
  */
  makeAtomComponentDefinition(name, roles, abstract=false) {
    ...
    let displayId;
    if (abstract && (name in this.SO_TERM_MAP)) {
      // if abstract part's role name does not match SO term, use SO term
      displayId = this.SO_TERM_MAP[name];
    } else { // if not an abstract part or name not in SO term map, use part id/role name
      displayId = name;
    }
    ...
    for (let role of roles) {
      ...
      } else if (role === 'ribozyme') { // one-off case; probably best to make an object to hold these if there are more later
        componentDefinition.addRole('http://identifiers.org/so/SO:0000374');
      } else {
        componentDefinition.addRole(`http://constellationcad.org#${role}`);
      }
    }
    componentDefinition.addType(SBOLDocument.terms.dnaRegion); //specifies the category of biochemical or physical entity
    if (!abstract) {
      componentDefinition.addSequence(`http://constellationcad.org/${name}_seq/${constants.VERSION}`);
    }
  ...
  makeSequenceAnnotation(orientation, dId, pId) {
    const saDID =  constants.SEQUENCE_ANNOTATION + this.sequenceAnnotationCount;
    const saPID =  pId + constants.DELIMITER + saDID;
    const sequenceAnnotation = this.doc.sequenceAnnotation(saPID + constants.DELIMITER + constants.VERSION);
    sequenceAnnotation.displayId = saDID;
    sequenceAnnotation.version = constants.VERSION;
    sequenceAnnotation.persistentIdentity = saPID;

    // create generic location to add to the sequence annotation
    const glDID = saDID + constants.GENERIC_LOCATION;
    const glPID =  saPID + constants.DELIMITER + glDID;
    let genLoc = this.doc.genericLocation(glPID + constants.DELIMITER + constants.VERSION);
    genLoc.displayId = glDID;
    genLoc.orientation = orientation;
    genLoc.version = constants.VERSION;
    genLoc.persistentIdentity = glPID;
    sequenceAnnotation.addLocation(genLoc);
  ...
  makeComponent(componentDefinition, name, template){
    ...
    for (let comp of template.components){
      if (JSON.stringify(comp.definition.uri) === JSON.stringify(componentDefinition.uri)){
  ...
  makeSBOLFromStack(stateStack, componentDefStack, fromOr=false){
    ...
    for (let stackObj of stateStack) {
      ...
      switch (key) {
        case constants.ATOM:
          if (!fromOr) {
            this.addAtomToStacks(stacks, stackObj, constants.ONE, null, null);
            ...
            stacks.componentDefs.push({operator: constants.ONE, name: stackObj[key], collections, orientation: stackObj.orientation});
          }
          break;
        case constants.OR:
