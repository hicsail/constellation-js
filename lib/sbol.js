    ...
    for (let key in stackObj){
      ...
      if (key === 'one-or-more'){
        ...
        if (templateStack.length > 0){
          ...
          componentDefStack.push({operator: [key], componentDef: componentDefinition, variantDV: []});
        }
      ...
      if (key === 'or'){
        ...
        if (templateStack.length > 0){
          ...
          tempVCStack.push({operator: 'one', component: component, variantDV: variantDerivations}); // keep track of components within another template
        }else{
          // else this is a new componentDef for the root
          componentDefStack.push({operator: 'one', componentDef: componentDefinition, variantDV: variantDerivations});
        }

        // add template for the OR to the temp stack
        ...
        stackObj[key].forEach(function (orRegion){
          ...
          let cdStack = [];
          let orStack = traverseStack(doc, orRegion, cdStack);
...
function generateCombinatorialSBOL(stateGraph, categories){
  ...
  let sbolStack = traverseStack(doc, stack, componentDefStack);
