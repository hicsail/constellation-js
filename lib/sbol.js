...
class sbolGeneration {
   ...
   */
  makeCollection(name) {
    const displayId = this.DESIGN_NAME + '_' + name + constants.COLLECTION;
    ...
    // * Currently set displayId to just the name for Constellation SBOL parsing
  ...
  */
  makeAtomComponentDefinition(name, roles, abstract=false) {
    ...
    const persistentId = constants.CONSTELLATION_URL + constants.DELIMITER + name;
