...
function makeStackFromStateGraph(stateGraph, stateStack, id){
  ...
  let queue = [];
  queue.push(id);
  console.log("\nin makestackfromstategraph", id);
  // console.log("\nstategraph\n", JSON.stringify(stateGraph, null, 2));
  while (queue.length !== 0){
    let id = queue.shift();

    // console.log("on id", id, " node", stateGraph[id].text);
    ...
    // To make SBOL generation easier, ignore the "zero" edge in the zero-or-one case.
    // if (stateGraph[id].operator.includes(ZERO_ONE)) {
    //   for (let edge of stateGraph[id].edges) {
    //     // console.log(stateGraph[edge].type);
    //     if (stateGraph[edge].type === graph.EPSILON) {
    //       continue;
    //     }
    //     queue.push(edge);
    //   }
...
function traverseOr(stateGraph, stateStack, id, endIds){
  ...
  stateGraph[id].edges.forEach(function (edge){
    ...
    // console.log("or substack", orSubStack);
...
function makeSBOLFromStack(doc, stateStack, componentDefStack){
  ...
  // console.log('FINAL collapsed version:');
