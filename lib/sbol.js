...
function handleOp(doc, stateGraph, templateComponentDefinition, id, op, operator){
  if (op === 'or'){
    ...
    //return a new component for the template
  ...
  if (op === 'zero-or-more' || op === 'one-or-more'){
    ...
    let cycleComponents = [];
    // returns the id of that last node in the cycle
    let lastId = handleCycle(doc, stateGraph, cycleTemplateComponentDefinition, cycleComponents, id, id);
    let thenFlag = addComponentsToTemplate(doc, stateGraph, cycleTemplateComponentDefinition, cycleComponents);

    // 3) CombinatorialDerivation - references #2
    //    a) VariableComponent - references #2a
    addVariableComponentsToCV(doc, cycleTemplateComponentDefinition, cycleComponents);

    //return a new component for the template
    const componentDefinition = makeComponentDefinition(doc, identity, false);
    const component = makeComponent(doc, componentDefinition, templateComponentDefinition.persistentIdentity, identity);
    if (thenFlag){
      return {op: 'then', component: component, lastId: lastId, operator: op};
    }
    return {op: op, component: component, lastId: lastId};
  }
}
function handleOperations(doc, stateGraph, templateComponentDefinition, id, operations){
  ...
  for(let op of operations){
    ...
    templateComponents.push(handleOp(doc, stateGraph, templateComponentDefinition, id, op, operator));
  }
...
/*
Helper function for "one-or-more" or "zero-or-more"
Visit edges starting from where the node the operator is on (call this startEdge)
Return is of the node that points back to the startEdge, for this is the last ID of the cycle
 */
function handleCycle(doc, stateGraph, templateComponentDefinition, cycleComponents, id, startEdge){
  if (stateGraph[id].edges.includes(startEdge)){
    if (!stateGraph[id].visited){
      handleTraverse(doc, stateGraph, templateComponentDefinition, cycleComponents, id);
    }
    return id;
  }
  for (let edge of stateGraph[id].edges){
    ...
    let nextId = handleTraverse(doc, stateGraph, templateComponentDefinition, cycleComponents, edge);
    return handleCycle(doc, stateGraph, templateComponentDefinition, cycleComponents, nextId, startEdge);
  }
}

/*
Helper function for how to handle every  node
If visited, return
If it has operations, handle all operations on that node, add whole component to array and return the id of the last node visited
If no operations and it is an atom, return the atom as a component to array and return its id
 */
function handleTraverse(doc, stateGraph, templateComponentDefinition, templateComponents, id){
  console.log(id);
  if (stateGraph[id].operator.length !== 0){
    stateGraph[id].visited = true;
    let componentObjs = handleOperations(doc, stateGraph, templateComponentDefinition, id, stateGraph[id].operator);
    componentObjs.forEach(function (componentObj){
      templateComponents.push(componentObj);
    });
    return componentObjs[componentObjs.length-1].lastId;
  }
  else if (stateGraph[id].operator.length === 0
    && stateGraph[id].type === graph.ATOM){
    stateGraph[id].visited = true;
    let atomText = stateGraph[id].text;
    let atomComponentDefinition = atomMap[atomText].componentDefinition;
    let componentObj = {op: 'none', component: makeComponent(doc, atomComponentDefinition, templateComponentDefinition.persistentIdentity, atomText), lastId:id};
    templateComponents.push(componentObj);
    return id;
  }
  return id;
...
function generateCombinatorialSBOL(stateGraph, categories){
  ...
  let rootComponents = [];
  traverseGraph(doc, stateGraph, rootComponentDefinition, rootComponents, firstId);
  ...
  addVariableComponentsToCV(doc, rootComponentDefinition, rootComponents);
