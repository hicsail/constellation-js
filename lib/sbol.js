...
class sbolGeneration {
  ...
  addToStacks(stacks, name, operator, variantDerivations, collections){
    ...
    const componentDefinition = this.makeComponentDefinition(name);
  ...
  */
  ...
  makeComponentDefinition(name, makeTemplate, makeRoot){
  ...
  makeSBOLFromStack(stateStack, componentDefStack){
    ...
    for (let stackObj of stateStack) {
      ...
      switch (key) {
        case constants.ATOM:
          this.addToStacks(stacks, stackObj[key], 'one', null, null);
          break;
        case constants.OR:
          ...
          let orCollections = [];
          this.addToStacks(stacks, orIdentity, 'one', variantDerivations, orCollections);
