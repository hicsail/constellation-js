      ...
      if (operation === 'or'){
        ...
        stateGraph[id].edges.forEach(function (edge){
          ...
          if (stateGraph[edge].type === graph.ATOM){
            tempStack.push({atom: stateGraph[edge].text});
          }
          stateGraph[edge].operator.forEach(function (operation){
            tempStack.push({[operation]: edge});
          });

          // then do a depth first search
          lastEdge = traverseOr(stack, tempStack, stateGraph, edge);
...
function traverseOr(stack, orStack, stateGraph, id){
  let lastEdge;
  stateGraph[id].visited = true;
  for (let edge of stateGraph[id].edges){
      if (stateGraph[edge].visited){
        if (stateGraph[edge].operator.includes('one-or-more')
          ...
          stack.push({end: edge}); //end the 'mores'
        }
        continue;
      }

      if (stateGraph[edge].type === graph.ATOM){
        orStack.push({atom: stateGraph[edge].text});
        stateGraph[edge].operator.forEach(function (operation){
          orStack.push({[operation]: edge});
        });
      }
      else{
        for (let operation of stateGraph[edge].operator){
          if (operation === 'then'){
            stateGraph[edge].visited = false; //reset
            return edge; //a 'then' that's not on the atom is not part of the OR
          }
          orStack.push({[operation]: edge});
        }
      }

    lastEdge = traverseOr(stack, orStack, stateGraph, edge);
  }
