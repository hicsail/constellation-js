...
const OPERATOR_URIS = {
  [handleOp.ZERO_SBOL]: 'http://sbols.org/v2#zeroOrMore',
  [handleOp.ONE]: 'http://sbols.org/v2#one',
  [handleOp.ONE_MORE]: 'http://sbols.org/v2#oneOrMore',
  [handleOp.ZERO_ONE_SBOL]: 'http://sbols.org/v2#zeroOrOne',
};
...
function getRootId(stateGraph){
  for (let id in stateGraph) {
    if (stateGraph[id].type === handleOp.ROOT){
      return id;
    }
  }
}
...
function collapseStack(stack){
  ...
  if(stack.length > 2){
    for(let i=0; i<stack.length-2; i++){
      let key = Object.keys(stack[i])[0];
      if (key === handleOp.ONE_MORE || key === handleOp.ZERO_SBOL || key === ZERO_ONE_SBOL){
...
function makeStackFromStateGraph(stateGraph, stateStack, id){
  ...
  while (queue.length !== 0){
    ...
    if (stateGraph[id].visited){
      if (stateGraph[id].type !== handleOp.ATOM
        && !endIds.includes(id)){
    ...
    if (stateGraph[id].type === handleOp.ACCEPT){
      continue;
    }
    if (stateGraph[id].type === handleOp.ATOM){
...
function traverseOrEdges(stateGraph, stack, subStack, id, endIds, moreStartId){
  ...
  if (stateGraph[id].visited){
    ...
    if (id === moreStartId){
      ...
      || stateGraph[id].operator.includes(handleOp.ZERO_SBOL)
  ...
  if (stateGraph[id].operator.length === 1 &&
    (stateGraph[id].operator[0] === handleOp.ONE_MORE ||
      stateGraph[id].operator[0] === handleOp.ZERO_SBOL ||
  ...
  let lastEdge;
  for (let operation of stateGraph[id].operator){
    if (stateGraph[id].type === handleOp.EPSILON
      && operation === handleOp.THEN){
...
function generateCombinatorialSBOL(stateGraph, categories, designName, numDesigns, maxCycles){
  ...
  Object.keys(stateGraph).forEach(function(id){
    ...
    if (atomType === handleOp.ATOM && !(atomText in atomMap)){
      ...
      makeAtomComponentDefinition(doc, atomText, categories[atomText].roles);
      categories[atomText].ids.forEach(function(variant){
        collection.addMember(makeAtomComponentDefinition(doc, variant, categories[atomText].roles));
