...
const OPERATOR_URIS = {
  [constants.ZERO_SBOL]: 'http://sbols.org/v2#zeroOrMore',
  [constants.ONE]: 'http://sbols.org/v2#one',
  [constants.ONE_MORE]: 'http://sbols.org/v2#oneOrMore',
  [constants.ZERO_ONE_SBOL]: 'http://sbols.org/v2#zeroOrOne',
};
...
function getRootId(stateGraph){
  for (let id in stateGraph) {
    if (stateGraph[id].type === constants.ROOT){
      return id;
    }
  }
}
...
function makeCollection(doc, name){
  const persistentId = constants.CONSTELLATION_URL + constants.DELIMITER + name + constants.COLLECTION;
...
function makeStackFromStateGraph(stateGraph, stateStack, id){
  ...
  while (queue.length !== 0){
    ...
    if (stateGraph[id].visited){
      if (stateGraph[id].type !== constants.ATOM
        && !endIds.includes(id)){
    ...
    if (stateGraph[id].type === constants.ACCEPT){
      continue;
    }
    if (stateGraph[id].type === constants.ATOM){
    ...
    let immediateCycles = stateGraph[id].edges.filter(e => stateGraph[e].visited);
...
function traverseOrEdges(stateGraph, stack, subStack, id, endIds, moreStartId){
  ...
  if (stateGraph[id].visited){
    ...
    else if (stateGraph[id].operator.includes(constants.ONE_MORE)
      || stateGraph[id].operator.includes(constants.ZERO_SBOL)
      || stateGraph[id].operator.includes(constants.ZERO_ONE_SBOL)){
      stack.push({end: id});
      endIds.push(id);
    }
    return null;
  ...
  if (stateGraph[id].operator.length === 1 &&
    (stateGraph[id].operator[0] === constants.ONE_MORE ||
      stateGraph[id].operator[0] === constants.ZERO_SBOL ||
      stateGraph[id].operator[0] === constants.ZERO_ONE_SBOL)){
    moreStartId = id;
  }

  let lastEdge;
  for (let operation of stateGraph[id].operator){
    if (stateGraph[id].type === constants.EPSILON
