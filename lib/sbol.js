...
class sbolGeneration {
  ...
  static createComponentDefObj(operator, componentDef, variantDVs, collections, name){
    return {
      operator: operator,
  ...
  addComponentsAndRestraintsToTemplate(templateCD, stack, cv, name=null, makeRoot=false){
    ...
    for(let stackObj of stack){
      // reassign the revComp-ed atoms with the ONE operator
      if (stackObj.operator === constants.REV_COMP) {
        stackObj.operator = constants.ONE;
      }
      ...
      if (stackObj.name in this.atomMap) {
        let seqAn = this.makeSequenceAnnotation(stackObj.name, templateCD.displayId, templateCD.persistentIdentity);
  ...
  */
  ...
  makeSequenceAnnotation(name, dId, pId) {
    ...
    genLoc.orientation = this.atomMap[name].locURI;
