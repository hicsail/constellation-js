...
const OPERATOR_URIS = {
  'http://sbols.org/v2#zeroOrMore': graphDataOnNodes.ZERO_MORE,
  'http://sbols.org/v2#one': graphDataOnNodes.ONE,
  'http://sbols.org/v2#oneOrMore': graphDataOnNodes.ONE_MORE
};
...
function createGraphFromSBOL(sbol) {
  ...
  //create graph
  recurseVariableComponents(rootCV, stateGraph, boundaryStack, categories);
  ...
  // Generate all paths
  const paths = graphDataOnNodes.enumeratePaths(root, stateGraph, maxCycles);
...
function reformatCategories(categories) {
  ...
  for (let name in categories) {
    ...
    newCategories[name] = {ids:cat, role:name};
  }
 ...
 * @param categories
 */
function recurseVariableComponents(combinatorialDerivation, stateGraph, boundaryStack, categories){
  ...
  for(let v=0; v<sortedVCs.length; v++){
    ...
    if (variantDerivs.length === 1 && !hasVariants){
      for (let cv of variantDerivs) {
        ...
        graphDataOnNodes.handleOp(OPERATOR_URIS[variableComponent.operator], stateGraph, boundaryStack);
      }
    }
    ...
    // else if (variantDerivs.length > 0) {
      ...
      // for (let c of collections){
        graphDataOnNodes.handleAtom([collections[c].displayId], stateGraph, boundaryStack, categories);
        ...
        if (c > 0 && boundaryStack.length > 1){
          graphDataOnNodes.handleOp(graph.OR, stateGraph, boundaryStack);
        }
      }
      ...
      for(let i=0; i<variantDerivs.length; i++){
        ...
        if (i > 0 || hasVariants){
          graphDataOnNodes.handleOp(graphDataOnNodes.OR, stateGraph, boundaryStack);
        }
      }
    ...
    if (v > 0 && boundaryStack.length > 1){
      graphDataOnNodes.handleOp(graphDataOnNodes.THEN, stateGraph, boundaryStack);
    }
