...
const OPERATOR_URIS = {
  'http://sbols.org/v2#zeroOrMore': constants.ZERO_MORE,
  'http://sbols.org/v2#zeroOrOne': constants.ZERO_ONE,
  'http://sbols.org/v2#one': constants.ONE,
  'http://sbols.org/v2#oneOrMore': constants.ONE_MORE
};
...
function createGraphFromSBOL(sbolArray, representation, combineMethod, tolerance) {
  ...
  if (Object.keys(allResults).length > 1) {
    ...
    handleOp.collapse(finalGraph, 'EDGE');
    ...
    let root = combineGraphs.findRoot(finalGraph);
    let paths = handleOp.enumerate(root, finalGraph, maxCycles, representation).paths;
    return {stateGraph:finalGraph, paths: paths, categories: finalCategories, numDesigns};
  } else {
...
function createGraphFromSingleSBOL(sbol, representation) {
  ...
  // Generate all paths
  const paths = handleOp.enumerate(root, stateGraph, maxCycles, representation).paths;
...
function recurseVariableComponents(combinatorialDerivation, stateGraph, boundaryStack, categories, representation){
  ...
  for(let v=0; v<sortedVCs.length; v++){
    ...
    if (v > 0 && boundaryStack.length > 1){
      handleOp.handleOp(constants.THEN, stateGraph, boundaryStack, representation);
    }
