...
function createGraphFromSBOL(sbolArray, representation, combineMethod, tolerance) {
  if ((sbolArray.length > 1) && (representation === constants.NODE)) {
    throw new Error(`The ${combineMethod} operation is not supported in the NODE representation`);
  }
  ...
  if (Object.keys(allResults).length > 1) {
    ...
    finalGraph = handleOp.collapse(finalGraph, representation);
  ...
  for(let v=0; v<sortedVCs.length; v++){
    ...
    // handle ORs
      ...
      for(let c=0; c<collections.length; c++) {
        stateGraph.handleAtom([formatName(collections[c].displayId)], partialBoundary, categories);
      }
...
function getCategories(sbol){
  ...
  for (let collection of sbol.collections){
    ...
    let catName = formatName(collection.displayId);
    categories[catName] = members;
  }
  return categories;
}

/**
 * Removes unnecessary "_collection" suffix from displayIds
 * @param displayId
 * @return {*}
 */
function formatName(displayId) {
  let nameSuffix = displayId.slice(displayId.length-constants.COLLECTION.length);
  if (nameSuffix === constants.COLLECTION) {
    return displayId.slice(0, displayId.length-constants.COLLECTION.length);
  } else {
    return displayId;
  }
}
