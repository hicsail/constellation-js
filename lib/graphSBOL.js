    ...
    // else if (variantDerivs.length > 0) {
      ...
      for(let c=0; c<collections.length; c++){
        ...
        if (c > 0 && boundaryStack.length > 1){
          handleOp.handleOp(handleOp.OR, stateGraph, boundaryStack, representation, categories, representation);
        }
      }
...
function getCategories(sbol){
  ...
  for (let collection of sbol.collections){
    ...
    categories[collection.displayId] = {ids: members, roles: roles};
  }
  return categories;
}
