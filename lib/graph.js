...
function getLeaves(nodeId, stateGraph, leaves) {
...
function deleteSubGraph(node, stateGraph) {
  var children = stateGraph[node].edges;
  
  for (var c of children) {
    deleteSubGraph(c, stateGraph);
  }
...
function andHelper(g, v, stateGraph, parentId) {
  ...
  var gChildren = [];
  var vChildren = [];
  popUpLayers(g.edges, stateGraph, gChildren);
  popUpLayers(v.edges, stateGraph, vChildren);

  for (var gChild of gChildren) {
    for (var vChild of vChildren) {
      if (gChild.text === vChild.text) {
        andHelper(gChild, vChild, stateGraph, newNodeId);
      }
    }
  }
}

function popUpLayers(edges, stateGraph, nodes) {
  for (var e of edges) {
    var node = stateGraph[e];
    if (node.type === EPSILON) {
      popUpLayers(node.edges, stateGraph, nodes);
    } else {
      nodes.push(node);
    }
  }
