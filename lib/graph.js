...
function handleThen(stateGraph, boundaryStack) {
  ...
  console.log("handleThen", "a: ", a.head, lenA, "b: ", b.head, lenB);
 ...
 * @param parentId The ID of the parent epsilon node
 */
// function handleZeroOrMoreold(stateGraph, boundaryStack) {
//
//   const a = boundaryStack.pop();
//
...
//
//   for (let i = 0; i < len; i++) {
//     let leaf = a.leaves.pop();
//     stateGraph[a.head].edges[i].dest = a.head;
//     delete stateGraph[leaf];
//   }
//
...
function handleZeroOrMore(stateGraph, boundaryStack) {
  ...
  for (var leaf of a.leaves) {
    stateGraph[leaf].edges.push({'src': leaf, 'dest': a.head, 'component': EPSILON, 'type': EPSILON, 'text': EPSILON });
  }
  let zero = createEpsilonNode(stateGraph);
  a.leaves.push(zero);
  stateGraph[a.head].edges.push({'src': a.head, 'dest': zero, 'component': EPSILON, 'type': EPSILON, 'text': 'ZERO'});
  stateGraph[a.head].operator.push(ZERO_MORE);
...
function collapseEpsilons(stateGraph, epsilonMap) {
  for (let epsilonId in epsilonMap) {
    ...
    const parentNode = stateGraph[parentIds[0]];
    const epsilonNode = stateGraph[epsilonId];

    for (let eEdge of epsilonNode.edges) {
      // if (eEdge.type !== EPSILON) {
      //   continue;
      // }
      // console.log("eEdge", eEdge);
      if (eEdge.dest in epsilonMap) {
        // if dest is also an epsilon, update epsilonMap
        // const index = Array.from(epsilonMap[eEdge.dest]).indexOf(epsilonId);
        // if (index > -1) {
        //   epsilonMap[eEdge.dest].delete(epsilonId);
        //   epsilonMap[eEdge.dest].add(parentIds[0]);
        // }
        delete epsilonMap[eEdge.dest];

        // parent --> epsilon now becomes parent --> epsilon's child
        for (let pEdge of parentNode.edges) {
          if (pEdge.dest === epsilonId) {
            pEdge.dest = eEdge.dest;
          }
        }
        if (parentNode.operator.includes(ONE_MORE)) {
          stateGraph[eEdge.dest].edges.push({'src': eEdge.dest, 'dest': parentIds[0], 'component': EPSILON, 'type': EPSILON, 'text': EPSILON});
        }
...
function getEpsilonParents(stateGraph) {
  ...
  for (let srcNode in stateGraph) {
    for (let edge of stateGraph[srcNode].edges) {
      if (stateGraph[edge.dest].type === EPSILON) {
        if (!(edge.dest in epsilonMap)) {
          epsilonMap[edge.dest] = new Set();
        }
        epsilonMap[edge.dest].add(srcNode);
      }
    }
  }
  return epsilonMap;
