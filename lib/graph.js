...
function addAcceptNodes(stateGraph, boundaryStack) {
  ...
  for (var leafId of leaves) {
    ...
    stateGraph[leafId].edges.push({'src': leafId, 'dest': acceptId, 'component': ACCEPT, type: ACCEPT, text: ACCEPT});
  }
  // console.log('bs',boundaryStack);
  ...
  //   // stateGraph[parentId].edges.push({'src': parentId, 'dest': epsilonId, 'component': atom[0].text, 'type': ATOM, 'text': atom[0].text });
...
function generateRootNode(stateGraph, boundaryStack) {
  ...
  const rootId = createEpsilonNode(stateGraph);
  stateGraph[rootId].text = ROOT;
  stateGraph[rootId].type = ROOT;

  addToBoundaryStack(rootId, [], boundaryStack);
...
function handleAtom(atom, stateGraph, boundaryStack) {
  const parent = boundaryStack.pop();
  const parentId = parent.head;

  const epsilonId = createEpsilonNode(stateGraph);
