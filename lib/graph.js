...
/**
 * Visits a node and recursively processes node's children
...
function visitNodes(nodeId, visited, stateGraph, currentPath, allPaths) {
  ...
  // Stop cycles
  if (visited[nodeId] && stateGraph[nodeId].type === ATOM) {
    return;
  }
  if (visited[nodeId] > 1) {
    return;
  ...
  for (let child of node.edges) {
    visitNodes(child, visited, stateGraph, currentPath, allPaths);
  }
...
function processPath(path, allPaths) {
  ...
  for (let node of path) {
    if (node.data.type === EPSILON || node.data.type === ROOT) {
      continue;
    }
    // Deep copy of node
