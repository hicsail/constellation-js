 ...
 * @param representation NODE or EDGE
 */
function handleOp(op, stateGraph, boundaryStack, representation) {
  if(op === ONE){
    return;
  }
  ...
  switch(representation){
    ...
    default: //edge
      ...
      if (op === OR) {
        graphDataOnEdges.handleOr(stateGraph, boundaryStack);
      }
      if (op === AND) {
        graphDataOnEdges.handleAnd(stateGraph, boundaryStack);
      }
      if (op === THEN) {
